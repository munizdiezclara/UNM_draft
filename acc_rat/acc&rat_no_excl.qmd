...
sme_acc_UNM07_test_condition <- acc_UNM07_test %>%
  group_by(predictiveness) %>%
...
There were no significant differences due to the main effect of *group*, `r apa(ANOVA_acc_UNM07_test, effect = "condition")`, `r report_BF_and_error(bay_ANOVA_acc_UNM07_test[1])`, nor of *predictiveness*: `r apa(ANOVA_acc_UNM07_test, effect = "predictiveness")`, `r report_BF_and_error(bay_ANOVA_acc_UNM07_test[2])`, but there was a significant *group x predictiveness* interaction, `r apa(ANOVA_acc_UNM07_test, effect = "condition:predictiveness")`, `r report_BF_and_error(bay_ANOVA_acc_UNM07_test_int[1])`. There was a simple main effect of *predictiveness* in the Certain group, *F*(`r sme_acc_UNM07_test_pred[1, 3]`, `r sme_acc_UNM07_test_pred[1, 4]`) = `r sme_acc_UNM07_test_pred[1, 5]`, *p* = `r sme_acc_UNM07_test_pred[1, 9]`, $\\eta^2_p$ = `r sme_acc_UNM07_test_pred[1, 8]`, but not in the Uncertain group, *F*(`r sme_acc_UNM07_test_pred[2, 3]`, `r sme_acc_UNM07_test_pred[2, 4]`) = `r sme_acc_UNM07_test_pred[2, 5]`, *p* = `r sme_acc_UNM07_test_pred[2, 9]`, $\\eta^2_p$ = `r sme_acc_UNM07_test_pred[2, 8]`.
...
# SME
#calculate the simple main effect of condition
sme_rat_UNM07_test_condition <- rat_UNM07_test %>%
  group_by(predictiveness) %>%
...
#calculate the simple main effect of predictiveness
sme_rat_UNM07_test_pred <- rat_UNM07_test %>%
  group_by(condition) %>%
  anova_test(rat ~ predictiveness + Error(pNum/predictiveness), effect.size = "pes") %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
...
# SME
#calculate the simple main effect of condition
sme_acc_UNM08_test_condition <- acc_UNM08_test %>%
  group_by(predictiveness) %>%
  anova_test(acc ~ condition, effect.size = "pes") %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
sme_acc_UNM08_test_condition #Call the output table
#calculate the simple main effect of predictiveness
sme_acc_UNM08_test_pred <- acc_UNM08_test %>%
  group_by(condition) %>%
  anova_test(acc ~ predictiveness + Error(pNum/predictiveness), effect.size = "pes") %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
...
There was a significant effect *predictiveness*, `r apa(ANOVA_rat_UNM08_test, effect = "predictiveness")`, `r report_BF_and_error(bay_ANOVA_rat_UNM08_test[2])`, but no significant effect of the *group*, `r apa(ANOVA_rat_UNM08_test, effect = "condition")`, `r report_BF_and_error(bay_ANOVA_rat_UNM08_test[1])`, nor of the *group x predictiveness* interaction, `r apa(ANOVA_rat_UNM08_test, effect = "condition:predictiveness")`, `r report_BF_and_error(bay_ANOVA_rat_UNM08_test_int[1])`. A simple main effects analysis showed a significant effect of *predictiveness* on the Certain Long group, *F*(`r sme_rat_UNM08_test_pred[1, 3]`, `r sme_rat_UNM08_test_pred[1, 4]`) = `r sme_rat_UNM08_test_pred[1, 5]`, *p* = `r sme_rat_UNM08_test_pred[1, 9]`, $\\eta^2_p$ = `r sme_rat_UNM08_test_pred[1, 8]`, and on the Certain Short group, *F*(`r sme_rat_UNM08_test_pred[2, 3]`, `r sme_rat_UNM08_test_pred[2, 4]`) = `r sme_rat_UNM08_test_pred[2, 5]`, *p* = `r sme_rat_UNM08_test_pred[2, 9]`, $\\eta^2_p$ = `r sme_rat_UNM08_test_pred[2, 8]`, but not on the Uncertain group, *F*(`r sme_rat_UNM08_test_pred[3, 3]`, `r sme_rat_UNM08_test_pred[3, 4]`) = `r sme_rat_UNM08_test_pred[3, 5]`, *p* = `r sme_rat_UNM08_test_pred[3, 9]`, $\\eta^2_p$ = `r sme_rat_UNM08_test_pred[3, 8]`.
