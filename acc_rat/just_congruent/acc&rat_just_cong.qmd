...
test2 <- filter(test2, congruence == "congruent")
#plot test accuracy
m_acc_test2 <- test2 %>%
  group_by(predictiveness, group) %>%
  summarise(mean_acc = mean(acc, na.rm = TRUE), 
...
There was no differences due to *predictiveness* in group Low similarity, `r apa(t.test_rat_test2_L)`, `r report_BF_and_error(bay_t.test_rat_test2_L[1])`.
...
d <- with(rat_test2_H,
          rat[predictiveness == "non-predictive"] - rat[predictiveness == "predictive"])
shapiro.test(d) #test the normality
t.test_rat_test2_H <- wilcox.test(rat ~ predictiveness, data = rat_test2_H, paired = TRUE)
...
For group High there was a significant effect of *predictiveness*, *V* = 21, *p* \< .001, `r report_BF_and_error(bay_t.test_rat_test2_H[1])`.
